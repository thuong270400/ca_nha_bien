...
One `Post` model serves both `/blog` (`type: NEWS`) and `/recipes` (`type: RECIPE`). `Post.content` is **HTML** written with Nuxt UI's `UEditor` (Tiptap) in `app/components/admin/PostForm.vue`, sanitized against an allowlist by `server/utils/sanitize-post.ts` (`sanitize-html`) in `post.service.ts` on every create/update — that's what makes the storefront's `v-html` in `StorefrontPostContent.vue` safe, so never write `content` without going through the service. Older posts may still hold plain text; `shared/utils/post-content.ts#toPostHtml` converts them on read (editor + storefront), no data migration needed. Inline content images are uploaded to the `posts` folder; `post.service.ts` deletes cover/inline images a post no longer references on update and all of them on delete. `publishedAt` is set the first time a post is published and kept across unpublish/republish.

`PostCategory` (optional `Post.categoryId`, `onDelete: SetNull`, but `deletePostCategory` refuses while posts still reference it) is managed in the "Danh mục bài viết" tab of `/admin/categories` (`AdminPostCategoryManager.vue`, `?tab=posts` opens it) and browsed on the storefront under "Góc Biển": `/goc-bien` (category grid + post search via `GET /api/posts?q=`) → `/goc-bien/[slug]` (`GET /api/posts?category=<slug>`, hidden categories 404/return nothing). Post detail pages stay at `/blog/[slug]` / `/recipes/[slug]` (by `type`, see `postPublicPath`); their breadcrumb goes through Góc Biển when the post has an active category. Public/admin post list endpoints omit `content`.
